...
var validateOrchestrateInput = (req, res, next) => {
  ...
  if (typeof stadium_name !== "string" || typeof current_match_phase !== "string" || typeof incident_report !== "string" || typeof current_crowd_density_level !== "string" || playing_teams && typeof playing_teams !== "string") {
    return res.status(400).json({
  ...
  const sanitizedReport = incident_report.replace(/<[^>]*>/g, "").replace(/javascript:/gi, "").replace(/[^\w\s.,!?:()''""-]/gi, "");
...
app.post("/api/orchestrate", rateLimiter, validateOrchestrateInput, async (req, res) => {
  ...
  if (!ai) {
    ...
    const mockResponse = {
      "incident_id": `STAD-2026-${Math.floor(1e3 + Math.random() * 9e3)}`,
      "severity_assessment": {
      ...
      "tactical_action_plan": {
        ...
        "staff_dispatch_assignment": isCritical ? "CRITICAL: Deploy 4x Medical Response Teams, 8x Security Marshals, and 12x Guest Experience volunteers to clear emergency lanes immediately." : "Deploy 6x Crowd Flow marshals and 4x Security Officers to manage turnstile gates and ease bottlenecking.",
        "crowd_flow_instruction": isCritical ? "RE-ROUTE ALERT: Direct all fans exiting the affected sector to adjacent low-density Sector 102 and Sector 108. Strictly avoid routing through North Gates (currently High density)." : "CROWD REGULATION: Temporarily pause ingress at Affected Gate. Divert subsequent queue streams to East Gate Annex."
      },
      "automated_broadcasts": {
        ...
        "spanish": `Atenci\xF3n aficionados de la Copa Mundial: Se ha producido un incidente en su sector. Mantengan la calma. Sigan las instrucciones del personal de seguridad y avancen lentamente hacia los sectores adyacentes 102 y 108. Gracias.`,
        "localized_team_language": `Achtung, Fu\xDFballfans: In Ihrem Bereich ist ein Zwischenfall aufgetreten. Bitte bewahren Sie Ruhe. Folgen Sie den Anweisungen des Sicherheitspersonals und gehen Sie langsam in die benachbarten Sektoren 102 und 108. Danke.`
      },
